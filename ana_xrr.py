...
path = os.getcwd()
#'Al2O3': 33.385,
cfg_model = {'bkg': 3e-8, 'dq': 0.05, 'scale': 1, 'q_offset': 0}
cfg_sld = {'al':22.425, 'nb':64.035, 'alox':18.419, 'nb2o5':17.819,'al2o3':33.319,'buff':20}
...
def make_layer(SLD_dict, cfg_mat, material, layer): 
    return SLD_dict[material](cfg_mat[layer]['thk'],cfg_mat[layer]['rough'])

def make_model(structure, cfg): 
    ...
    model.scale.setp(bounds = (0.9, 1.35), vary = cfg_vary['scale']) # maximum value
...
def sim_al(file, level='alox/al', qrng=[0.025, 0.3], save_name='test', cfg_in={}, cfg_in2={}, cfg_sld_in={}, run_mc=False, SLD_rng=[1,0.3], plot=True, no_fit=False, careful=False):
    cfg_mat = {'nb':{'rough':6,'thk':80},'al': {'rough':1, 'thk':300},'al_top': {'rough':1, 'thk':370}, 
               'top_oxide':{'rough':1, 'thk':22},'buffer':{'rough':2, 'thk':5},'JJ':{'rough':2, 'thk':20},'bottom_oxide':{'rough':1, 'thk':5}, 
               'substrate':{'rough':0.5,'rough_max':15}}
    ...
    al = make_layer(SLD_dict, cfg_mat, 'Al', 'al')
    nb = make_layer(SLD_dict, cfg_mat, 'Nb', 'nb')
    al_top = make_layer(SLD_dict, cfg_mat, 'Al', 'al_top')

    bottom_oxide = make_layer(SLD_dict, cfg_mat, 'AlOx_bot', 'bottom_oxide')
    top_oxide = make_layer(SLD_dict, cfg_mat, 'AlOx_top', 'top_oxide')
    jj = make_layer(SLD_dict, cfg_mat, 'AlOx_JJ', 'JJ')

    buffer = make_layer(SLD_dict, cfg_mat, 'Buff_top', 'buffer')
    jj_buffer_top = make_layer(SLD_dict, cfg_mat, 'Buff_JJ_top', 'buffer')
    jj_buffer_bot = make_layer(SLD_dict, cfg_mat, 'Buff_JJ_bot', 'buffer')
    ...
    if level == 'al': 
        ...
        ox_layers = [1,3]
        met_layers=[2,4]
    elif level == 'alox/al/alox/al/alox': 
        structure = air | top_oxide | al_top | jj | al | bottom_oxide | substrate
        ox_layers = [1,3,5]
        met_layers=[2,4]
    ...
    for i in ox_layers:
        ...
        if careful: 
            ...
            s.rough.setp(bounds = (0,20), vary = True)                    
...
def plot_obj(obj, labs):
    ...
    plt.loglog(obj[0].data.x, obj[0].data.y, '.-', markersize=4, label='Data', color='k')
